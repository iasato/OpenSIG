...
public class EnviarNfeInutilizada extends Chain {

	private FiscalServiceImpl servico;
	private FisNotaSaida saida;

	public EnviarNfeInutilizada(Chain next, FiscalServiceImpl servico, FisNotaSaida saida) throws OpenSigException {
		super(next);
	...
	public void execute() throws OpenSigException {
		try {
			...
			if (doc.getElementsByTagName("Signature").item(0) == null) {
				// assina
				xml = NFe.assinarXML(doc, ENotaStatus.INUTILIZANDO, saida.getEmpEmpresa());
			}
			// valida
			String xsd = UtilServer.getRealPath(UtilServer.CONF.get("nfe.xsd_inutilizando"));
			...
			if (ret.getInfInut().getCStat().equals("102")) {
				...
				saida.setFisNotaSaidaXml(montaProcInutNfe(saida.getFisNotaSaidaXml(), inut));
			} else {
	...
	public static String montaProcInutNfe(String inut, String proc) throws OpenSigException {
		...
		sb.append("<ProcInutNFe versao=\"" + UtilServer.CONF.get("nfe.versao") + "\" xmlns=\"http://www.portalfiscal.inf.br/nfe\">");
